# This script combines preprocessing, model definition, compilation, training, and evaluation.
import numpy as np
...
print("Starting the complete fine-tuning and evaluation process...")

# 1. Load and preprocess the CIFAR-10 dataset
print("Step 1: Loading and preprocessing CIFAR-10 dataset...")
(x_train_full, y_train_full), (x_test, y_test_orig) = cifar10.load_data() # y_test_orig for later use if needed
...
print("- EarlyStopping callback configured (monitor='val_loss', patience=3).")
...
model_save_path = "/home/ubuntu/cifar10_vgg16_finetuned.keras" # Use .keras for new format
...
# Load the model (optional, as it's already in memory, but good practice for a full script)
# loaded_model = load_model(model_save_path)
# print("- Model loaded from disk for evaluation.")

# Evaluate the model (using the model in memory)
test_loss, test_accuracy = model.evaluate(x_test, y_test, verbose=1)
print(f"- Test Loss: {test_loss}")
print(f"- Test Accuracy: {test_accuracy}")

# Check if performance is better than random (10 classes, so random is 0.1)
if test_accuracy > 0.1:
    print("- Model performance is better than random guessing (10%).")
else:
    print("- Model performance is not better than random guessing. Consider retraining or hyperparameter tuning.")
print("Step 5 finished.")

print("\nAll steps for training and evaluation completed.")
